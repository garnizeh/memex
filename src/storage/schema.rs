...
mod tests {
    ...
    fn test_initialize_schema_idempotent() {
        ...
        initialize_schema(&conn)
            .expect("second schema initialization should succeed without error");
        ...
        assert!(
            table_names.contains("vec_chunks"),
            "vec_chunks table missing"
        );
        ...
        assert!(
            index_names.contains("idx_chunks_doc"),
            "idx_chunks_doc index missing"
        );
        assert!(
            index_names.contains("idx_chunks_parent"),
            "idx_chunks_parent index missing"
        );
        assert!(
            index_names.contains("idx_chunks_type"),
            "idx_chunks_type index missing"
        );
        assert!(
            index_names.contains("idx_edges_target"),
            "idx_edges_target index missing"
        );
        assert!(
            index_names.contains("idx_edges_type"),
            "idx_edges_type index missing"
        );
    }
    ...
    fn test_vec_chunks_insert_and_query() {
        ...
        let serialized_vec: Vec<u8> = dummy_vec.iter().flat_map(|f| f.to_le_bytes()).collect();
        ...
        let count: i64 = conn
            .query_row(
                "SELECT count(*) FROM vec_chunks WHERE chunk_id = 'chunk-1'",
                [],
                |r| r.get(0),
            )
            .unwrap();
