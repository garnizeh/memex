...
fn test_simple_fixtures_exist_and_parse() {
    ...
    assert_eq!(
        discovered.len(),
        3,
        "simple/ should contain exactly 3 Markdown files"
    );
    ...
    for path in &discovered {
        ...
        assert!(
            !content.is_empty(),
            "simple file should not be empty: {:?}",
            path
        );
        ...
        assert!(
            !chunks.is_empty(),
            "simple file should produce chunks: {:?}",
            path
        );
    }
}
...
fn test_complex_fixtures_discovery_and_gitignore() {
    ...
    assert!(
        complex_dir.exists(),
        "complex fixtures directory must exist"
    );
    ...
    for path in &discovered {
        ...
        assert!(
            !chunks.is_empty(),
            "complex file should produce chunks: {:?}",
            path
        );
    }
}
...
fn test_edge_cases_fixtures() {
    ...
    assert!(
        edge_dir.exists(),
        "edge_cases fixtures directory must exist"
    );
    ...
    assert!(
        chunks.len() >= 6,
        "deeply_nested.md should parse H1 through H6 hierarchy"
    );
    ...
    assert!(
        !chunks.is_empty(),
        "broken_links.md should parse without panic"
    );
    ...
    assert!(
        !chunks.is_empty(),
        "no_headings.md should produce fallback chunks"
    );
    ...
    assert!(
        ...
        "large_file.md should be substantial in size (~1MB)"
    );
    ...
    assert!(
        chunks.len() > 1000,
        "large_file.md should produce thousands of chunks"
    );
}
